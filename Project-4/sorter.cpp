...
//Global functions
void InitVector(vector<int>& item_vetor, int size); void PrintVector(const vector<int>& item_vector, string name); int Elapsed(const timeval &start, const timeval &end);
...
void InitVector(vector<int>& item_vector, int rand_max)
{
     ...
     for (int i = 0; i < rand_max; i++)
     {
          spot = rand() % (pool.size());           item_vector[i] = pool[spot];
